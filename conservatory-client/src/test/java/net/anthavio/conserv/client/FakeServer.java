...
import net.anthavio.conserv.client.Jackson2ConfigParser;
import net.anthavio.conserv.client.JaxbConfigParser;
...
public class FakeServer extends HttpServlet {
	...
	*/
	...
	private Server setup(boolean authentication) {
		...
		if (authentication) {
			server.setHandler(securityHandler); //use security
			securityHandler.setHandler(servletHandler); //chain the servlet 
		} else {
			server.setHandler(servletHandler); //use servlet directly
		}
